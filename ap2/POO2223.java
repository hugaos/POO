...
public class POO2223 {
	...
	// CREATE THE CLASS DEFINITIONS ACCORDING TO THE DESCRIPTION IN THE EXAM AND THE
	// CODE BELOW
	private static void alinea1(PrintStream out) {
		...
		storageService.registerClient(123456789, "João Sousa", "PERSONAL");
		storageService.registerClient(234567890, "Ana Pereira", "PERSONAL");
		storageService.registerClient(501234567, "Arlindo Marques, Unipessoal", "ENTERPRISE");
		storageService.registerClient(501987654, "A Tralha", "ENTERPRISE");
	...
	private static void alinea2(PrintStream out) {
		...
		// Registering clients
		storageService.registerClient(123456789, "João Sousa", "PERSONA");
		storageService.registerClient(150987654, "Ana Pereira", "PERSONAL");
		storageService.registerClient(157894320, "Carla Moura", "PERSONAL");
		storageService.registerClient(501234567, "Arlindo Marques, Unipessoal", "ENTERPRISE");
		storageService.registerClient(501987654, "A Tralha", "ENTERPRISE");
		storageService.registerClient(505321987, "Patudos, LDA", "ENTERPRISE");
		...
		/*
		 ** Adicione a seguir o código necessário para a leitura do ficheiro e registo
		 * dos alugueres.
		 ...
		 * programa.
		 ** Do not modify the method from this point on. You may comment to excute the
		 * programme.
		 */

		out.println("\n" + storageService + ": Finished reading file.\n\n");

		// Deve devolver uma lista vazia (ou null) pois não existem garagens com a
		// dimensão indicada (ou maiores)
		// This should return empty (or null) since there are no garage units with the
		// required dimensions (or larger)
		out.println("Looking for garage with dimensions 300x400x300");
		List<StorageUnit> available = storageService.findAvailableUnits(UnitType.GARAGE, LocalDate.of(2023, 6, 1), 90,
				new int[] { 300, 400, 300 });
		...
		available = storageService.findAvailableUnits(UnitType.GARAGE, LocalDate.of(2023, 6, 1), 90,
				new int[] { 300, 400, 280 });
		if (available == null || available.isEmpty()) {
			out.println("** No options available for the type, dimensions, and dates requested.\n");
		} else {
		...
		// listar todas as unidades disponíveis a partir de 2023/12/01 por 100 dias, por
		// ordem decrescente de volume total
		// list all available units from 2023/12/01 for 100 days, in decreasing order of
		// total volume
